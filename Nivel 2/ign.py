...
class IGNCrawler(CrawlSpider):
    ...
    rules = (
        # Primera regla para cambiar entre las pestañas
        Rule(
            LinkExtractor(
                allow=r'type='
            ), follow=True
        ), # HORIZONTALIDAD POR TIPO => No tiene callback ya que aqui no voy a extraer datos
        # Segunda regla para cambiar de paginas
        Rule(LinkExtractor(
            allow=r'&page=\d+'
        ), follow=True
        ...
        # Las siguientes reglas sirven para extrar los datos
        Rule(
